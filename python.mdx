...
#### `__init__(on_payment_request: Optional[Callable] = None, api_url: Optional[str] = None)`
<ParamField body="on_payment_request" type="function" required>
This function is called when a payment is required. It receives an `offer` object containing details about the payment, such as amount, currency, description, and the Lightning invoice. You can implement your own logic to handle payments here.
...
#### `convert(markdown: str, date: Optional[str] = None, title: Optional[str] = None, download_path: Optional[str] = None, return_bytes: bool = False)`
