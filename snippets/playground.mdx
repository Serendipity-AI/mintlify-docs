export const MarkdownToPDFPlayground = () => {
  ...
  const STATUS_MESSAGES = {
    ...
    GENERATING: 'Payment received! Generating your PDF...',
  ...
  const resetState = (shouldCancel = false) => {
    if (shouldCancel) {
      ...
      console.log('Explicitly cancelling operation');
    }
  ...
  const convert = async () => {
    ...
    console.log('Starting convert, isCancelledRef:', isCancelledRef.current);
    resetState(false);
  ...
  const pollPayment = async (payload) => {
    ...
    try {
      ...
      console.log('Payment error, setting isProcessing to false');
  ...
  const showMarkdown = !showPaymentUI && !showDownloadUI && !isProcessing;
    ...
    <div className="max-w-2xl mx-auto space-y-4">
      ...
      <div className="border border-zinc-200 dark:border-zinc-800 rounded-2xl p-6 space-y-4">
        ...
        <p className="prose m-0 font-normal text-sm leading-6 text-gray-600 dark:text-gray-400">
          ...
          {showMarkdown && 
            <textarea 
              className="w-full h-40 font-mono text-sm p-4 rounded-2xl border border-zinc-200 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-900 text-zinc-900 dark:text-zinc-100 placeholder-zinc-400 dark:placeholder-zinc-500 focus:outline-none focus:border-indigo-600 focus:ring-1 focus:ring-indigo-200 dark:focus:border-indigo-400 dark:focus:ring-indigo-900 transition"
              value={markdown}
          ...
          {showPaymentUI && (
            <div className="bg-zinc-50 dark:bg-zinc-900/50 rounded-2xl p-6 border border-zinc-200 dark:border-zinc-800 space-y-4 flex flex-col items-center">
              <div className="flex flex-col items-center w-full space-y-3">
                <span className="text-base font-medium text-zinc-900 dark:text-zinc-100">Scan to Pay</span>
                <img
                  src={qrSvg}
                  alt="Lightning QR code"
                  className="w-40 h-40 bg-white rounded-2xl border border-zinc-100 dark:border-zinc-800"
                />
                <div className="flex items-center w-full bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-2xl px-3 py-2 font-mono text-xs text-zinc-800 dark:text-zinc-200">
                  <span className="flex-1 break-all pr-2 leading-tight">{invoice}</span>
                  <button
                    onClick={copyInvoice}
                    className="p-1.5 rounded-2xl hover:bg-indigo-100 dark:hover:bg-indigo-900 transition"
          ...
          {showDownloadUI && (
            <div className="bg-zinc-50 dark:bg-zinc-900/50 rounded-2xl p-6 border border-zinc-200 dark:border-zinc-800 space-y-4 flex flex-col items-center">
              <span className="text-base font-medium text-zinc-900 dark:text-zinc-100">{statusMessage}</span>
              <div className="w-full space-y-2">
                <button
                  onClick={downloadDocument}
                  className="w-full px-4 py-3 rounded-2xl font-medium
                ...
                <button
                  onClick={resetState}
                  className="w-full px-3 py-2 rounded-2xl bg-zinc-100 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 text-zinc-700 dark:text-zinc-200 text-sm font-medium hover:bg-zinc-200 dark:hover:bg-zinc-700 transition"
                >
                  Convert Another
                </button>
              </div>
              ...
              <div className="relative z-10 flex items-center justify-center gap-2">
                {isProcessing ? (
                  <>
                    ...
                    <span className="group-hover:hidden transition-opacity duration-200">{statusMessage} ({Math.round(progress)}%)</span>
